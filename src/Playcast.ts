...
import {
  ...
  NewPlaycastBlock,
  NewPlaycastEvent,
  PlaycastBaseResponse,
  PlaycastBlockType,
  PlaycastCommandQueue,
  PlaycastConnectionConfig,
  PlaycastEvent,
  PlaycastUnitConfig,
  PlaycastUnitEof,
  PlaycastUnitStatus,
  PlaycastWatermarkStatus,
} from './types';
...
export default class Playcast {
  ...
  constructor(
    connectionConfig: PlaycastConnectionConfig = {
      host: 'localhost',
      port: 8383,
    },
  ) {
  ...
  setupControls(autoRoll: {
    after: number;
    enabled: boolean;
  }): Promise<PlaycastBaseResponse & { controls?: Record<string, unknown> }> {
  ...
  ): Promise<PlaycastBaseResponse & { primary?: Record<string, unknown> }> {
  ...
  onStatusUpdate(
    callback: (status: {
      unit?: PlaycastUnitStatus;
      updates: PlaycastEvent[];
      commandQueue: PlaycastCommandQueue;
    }) => void,
  ): void {
  ...
  getStatus(): Promise<
    PlaycastBaseResponse & {
      configuredUnits?: PlaycastUnitConfig;
      units?: PlaycastUnitStatus;
    }
  > {
  ...
  listUnit(
    unit: number,
  ): Promise<PlaycastBaseResponse & { unit?: PlaycastUnitStatus }> {
    return this._socket.emit('list-unit', { unit });
  }

  listUnits(): Promise<
    PlaycastBaseResponse & { units?: Record<number, PlaycastUnitStatus> }
  > {
  ...
  append(
    unit: number,
    event: NewPlaycastEvent,
  ): Promise<PlaycastBaseResponse & { event?: PlaycastEvent }> {
    return this._socket.emit('append', { unit, event });
  }

  appendMany(
    unit: number,
    events: NewPlaycastEvent[],
  ): Promise<PlaycastBaseResponse & { events?: PlaycastEvent[] }> {
  ...
  insertMany(
    unit: number,
    events: { event: NewPlaycastEvent; targetEvent: string }[],
    targetEvent?: string,
  ): Promise<
    PlaycastBaseResponse & {
      events?: { event: PlaycastEvent; targetEvent?: string }[];
    }
  > {
  ...
  update(
    unit: number,
    event: NewPlaycastEvent & { id: string },
  ): Promise<PlaycastBaseResponse & { event?: PlaycastEvent }> {
  ...
  moveMany(
    ...
    return this._socket.emit('move-many', {
      unit,
      firstEvent,
      howMany,
      targetEvent,
  ...
  copyPaste(
    ...
    return this._socket.emit('copy-paste', {
      unit,
      sourceUnit,
      events: eventIds,
      targetEvent,
  ...
  loadPlaylist(
    ...
    return this._socket.emit('load-playlist', {
      unit,
      filepath,
      fromBeginning,
    });
  ...
  createBlock(
    unit: number,
    blockInfo: NewPlaycastBlock,
    firstEventId: string,
    lastEventId?: string,
  ): Promise<PlaycastBaseResponse> {
    return this._socket.emit('create-block', {
      unit,
      blockInfo,
      firstEventId,
      lastEventId,
  ...
  updateBlock(
    unit: number,
    blockInfo: NewPlaycastBlock & { id: string },
  ): Promise<PlaycastBaseResponse> {
  ...
  getWatermarks(): Promise<
    PlaycastBaseResponse & Record<number, PlaycastWatermarkStatus>
  > {
  ...
  setWatermark(
    ...
    return this._socket.emit('set-watermark', {
      unit,
      filepath,
      geometry,
      enabled,
  ...
  loadCustomRundown(
    ...
    loader: string,
    rundown: string,
    blockType?: PlaycastBlockType,
  ): Promise<PlaycastBaseResponse> {
    return this._socket.emit('load-custom-rundown', {
      unit,
      loader,
      rundown,
      blockType,
  ...
  getPlaylists(
    unit: number,
  ): Promise<PlaycastBaseResponse & { playlists?: string[] }> {
    return this._socket.emit('get-playlists', { unit });
  }

  getCustomActions(
    unit: number,
  ): Promise<PlaycastBaseResponse & { actions?: string[] }> {
    return this._socket.emit('get-custom-actions', { unit });
  }

  getRundownLoaders(
    unit: number,
  ): Promise<PlaycastBaseResponse & { loaders?: string[] }> {
    return this._socket.emit('get-rundown-loaders', { unit });
  }

  getPlayoutReportLoaders(
    unit: number,
  ): Promise<PlaycastBaseResponse & { loaders?: string[] }> {
  ...
  getPlayoutReport(
    ...
    to?: number,
    columns?: string[],
    customReport?: string | 'default',
  ): Promise<PlaycastBaseResponse & { report?: string }> {
    return this._socket.emit('get-playout-report', {
      unit,
      from,
      to,
      columns,
